...
def discriminator(x):
    ...
    minibatch = custom_layers.minibatch_layer(h_pool2_flat, initializer, 10, 6)
...
loss_d = tf.reduce_mean(Dx) - tf.reduce_mean(Dg) #This optimizes the discriminator.
...
d_clipped = [d.assign(tf.clip_by_value(d, -0.01, 0.01)) for d in d_params]
