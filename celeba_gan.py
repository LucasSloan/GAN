...
def load_celeba(batch_size):
    image_files_dataset = tf.data.Dataset.list_files("/home/lucas/training_data/celeba/img_align_celeba/*")
...
class CELEBA_GAN(GAN):
    def __init__(self, training_steps, batch_size, label_based_discriminator, output_real_images = False):
        super().__init__(64, 64, "celeba", training_steps, batch_size, label_based_discriminator, output_real_images = False)
    ...
    def load_data(self, batch_size):
        x = load_celeba(batch_size).get_next()
        x.set_shape([batch_size, 64, 64, 3])
        return x, None, None

g = CELEBA_GAN(200000, 100, False)
g.run()